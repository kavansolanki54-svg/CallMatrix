    ...
    public class CallService : ICallService
    {
        ...
        public async Task<ApiResponse<CallRecordingResponse>> SaveCallRecordingAsync(UploadCallRecordingRequest request, System.IO.Stream? fileStream, string? fileExtension, int employeeId)
        {
            ...
            try
            {
                ...
                long actualFileSize = request.FileSize;
                ...
                if (fileStream != null && fileStream.Length > 0)
                {
                    try
                    {
                        ...
                        // Save incoming stream to a temp file
                        var tempFileName = $"{cleanPhone}_{call.CallId}_temp{extension}";
                        var tempFilePath = System.IO.Path.Combine(uploadsFolder, tempFileName);
                        System.Console.WriteLine($"[CallService] SaveCallRecordingAsync saving temporary upload to: {tempFilePath}");
                        using (var targetStream = new System.IO.FileStream(tempFilePath, System.IO.FileMode.Create))
                        {
                            await fileStream.CopyToAsync(targetStream);
                        }

                        var finalExtension = ".mp3";
                        var finalFileName = $"{cleanPhone}_{call.CallId}{finalExtension}";
                        filePath = System.IO.Path.Combine(uploadsFolder, finalFileName);

                        bool isTranscoded = false;
                        try
                        {
                            string ffmpegExecutable = System.Runtime.InteropServices.RuntimeInformation.IsOSPlatform(System.Runtime.InteropServices.OSPlatform.Windows) ? "ffmpeg.exe" : "ffmpeg";
                            var ffmpegDir = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ffmpeg");
                            if (!System.IO.Directory.Exists(ffmpegDir))
                            {
                                var projectRoot = System.IO.Directory.GetParent(AppDomain.CurrentDomain.BaseDirectory)?.Parent?.Parent?.FullName;
                                if (projectRoot != null)
                                {
                                    ffmpegDir = System.IO.Path.Combine(projectRoot, "ffmpeg");
                                }
                            }
                            string ffmpegBinaryPath = System.IO.Path.Combine(ffmpegDir, ffmpegExecutable);

                            if (System.IO.File.Exists(ffmpegBinaryPath))
                            {
                                System.Console.WriteLine($"[CallService] SaveCallRecordingAsync: Transcoding temp file to MP3: {tempFilePath} -> {filePath}");
                                var mediaInfo = await Xabe.FFmpeg.FFmpeg.GetMediaInfo(tempFilePath);
                                var conversion = Xabe.FFmpeg.FFmpeg.Conversions.New()
                                    .AddStream(mediaInfo.Streams)
                                    .AddParameter("-b:a 32k") // 32 kbps bitrate
                                    .AddParameter("-ar 16000") // 16 kHz sample rate
                                    .AddParameter("-ac 1")     // 1 channel (mono)
                                    .SetOutput(filePath);

                                await conversion.Start();
                                isTranscoded = true;
                                System.Console.WriteLine($"[CallService] SaveCallRecordingAsync: Transcoding completed. Output size: {new System.IO.FileInfo(filePath).Length} bytes");
                            }
                            else
                            {
                                System.Console.Error.WriteLine($"[CallService] SaveCallRecordingAsync warning: FFmpeg binary not found at '{ffmpegBinaryPath}'. Falling back to direct save.");
                            }
                        }
                        catch (System.Exception ex)
                        {
                            System.Console.Error.WriteLine($"[CallService] SaveCallRecordingAsync transcoding exception: {ex.Message}\n{ex.StackTrace}");
                        }

                        if (!isTranscoded)
                        {
                            // Fallback: use the original file directly
                            var fallbackFileName = $"{cleanPhone}_{call.CallId}{extension}";
                            filePath = System.IO.Path.Combine(uploadsFolder, fallbackFileName);
                            ...
                            System.Console.WriteLine($"[CallService] SaveCallRecordingAsync: Fallback direct save completed: {filePath}");
                        }
                        else
                        {
                            // Clean up temp file
                        ...
                        var relativeFileUrl = System.IO.Path.Combine(relativeDir, finalFileName).Replace("\\", "/");
                        fileUrl = $"/{relativeFileUrl}";

                        // Update request properties for mapper and database updates
                        request.FileName = finalFileName;
                        request.FileSize = actualFileSize;
